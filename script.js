...
const formatMovementDates = function (movDate) {
  ...
  const daypassed = calcDayPassed(new Date(), movDate);
  if (daypassed === 0) return `Today`;
  else if (daypassed === 1) return `Yesterday`;
  else if (daypassed < 7) return `${daypassed} day's ago`;
  else
    return Intl.DateTimeFormat(currentAccount.locale, options).format(movDate);
};

const formatNumber = function (value, locale, valType) {
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const formatedMov = formatNumber(
      mov,
